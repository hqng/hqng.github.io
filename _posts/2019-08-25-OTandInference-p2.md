...
{% endraw %}
<br>
The term $ \mathbb{E}_{ q_{ \boldsymbol{\phi} } (z|x_i)} \left[ \log p_{\boldsymbol{\theta}}( x_i|z ) \right] $ is more tricky because we want both its (estimated) value and gradient w.r.t {% raw %} $ \phiparam $ {% endraw %} AAAAAA.
