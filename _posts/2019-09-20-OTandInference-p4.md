...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{SWAESlicedDist}
	\captionsetup{width=.75\textwidth}
	\caption{Distribution slicing (Source: \cite{S-WAE})}
	\label{fig4.1}
\end{figure}
...
\begin{comment}
To numerically compute Wasserstein distance between one-dimensional distributions, we consider 2 scenarios.
The first case is we know cumulative distribution functions $P_X$, $P_Y$ of 2 densities $p_X$, $p_Y$ respectively.  (\ref{eq4.4}) can be analytically calculated by using $ \frac{1}{M} \sum_{m=1}^{M} a_m $ with $a_m = c(P_X^{-1}(\tau_m), P_Y^{-1}(\tau_m))$, $\tau_m = \frac{2m-1}{2M} $.
\end{comment}
Thus if every sliced Wasserstein distances are calculated, then by (\ref{eq4.6}), Wasserstein distance of non-projected distributions can be accomplished. \\
...
On the other hand, if $p_X$ and $p_Y$ are already known, (\ref{eq4.6}) can be numerically calculated without their samples by using  $ \frac{1}{M} \sum_{m=1}^{M} a_m $ with $a_m = c(P_X^{-1}(\tau_m), P_Y^{-1}(\tau_m))$, $\tau_m = \frac{2m-1}{2M}$ (see the figure \ref{fig4.2}).

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{SWAENumerics}
	\captionsetup{width=.8\textwidth}
	\caption{Top row: one-dimensional distribution densities are known (top left), the Wasserstein distance then can be analytically computed (top right). Bottom row: only samples of distributions are available (bottom left), then $a_m = c(x_{i[m]}. y_{j[m]} ) $ where $x_{i[m]}. y_{j[m]}$ are sorted in ascending order (bottom right). (Source: \cite{S-WAE})  }
	\label{fig4.2}
\end{figure}
...
\begin{mytheorem} \label{thrm4.5}
	Let $W_c(p_n, q_n)$ is the Wasserstein distance between two empirical distributions $p^*, q^*$. For $n$ tends to infinity, there exists a positive number $s$ such that:
	\begin{align*}
		\E_{pq}\left[ W_c(p_n, q_n) \right] \lesssim W_c(p,q) + n^{-1/s}
	\end{align*}
\end{mytheorem}
See \cite{WVI} for its proof.
...
\end{align}
It is clear that $\E_{pq}[\tilde{\mathcal{L}}_c(p_n, q_n)] = 0 $ if $p=q$ and furthermore, $\lim_{n \rightarrow \infty}\tilde{\mathcal{L}}_c(p_n, q_n) = \mathcal{L}(p,q) $.
...
\end{align}
where $h_q(x)$ and $g_p(z)$ are deterministic functions represent for encoder and decoder respectively.\\
...
\end{align}
Note that when $f$ satisfies above condition, $C_f^{p,q}(\cdot,\cdot)$ in fact is a valid Wasserstein distance \cite{WVI}.
